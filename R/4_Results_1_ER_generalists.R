...
ext.risk.mammals.data<-data.frame(or=ext.risk.mammals.gain.excluded.na.omit$order, 
                                  pm=ext.risk.mammals.gain.excluded.na.omit$patch_matrix,
                                  rpm=ext.risk.mammals.gain.excluded.na.omit$change_patch_matrix,
                                  cm=ext.risk.mammals.gain.excluded.na.omit$continuum_p95,
                                  rcm=ext.risk.mammals.gain.excluded.na.omit$change_continuum_p95,
                                  hm=ext.risk.mammals.gain.excluded.na.omit$hybrid_p95,
                                  rhm=ext.risk.mammals.gain.excluded.na.omit$change_hybrid_p95,
...
rpmrcmrhm.effectsize.table$habitat_breadth <- "Generalists"

# Save effect sizes
write.table(pmcmhm.effectsize.table, file="effectsize_pmcmhm_generalists.csv", sep="\t", row.names=F)
write.table(rpmrcmrhm.effectsize.table, file="effectsize_rpmrcmrhm_generalists.csv", sep="\t", row.names=F)
...
effect.pmcmhm$habitat_breadth <- "Generalists"

# Save predicted probabilities
write.table(effect.pmcmhm, file="predicted_probability_pmcmhm_generalists.csv", sep="\t", row.names=F)
