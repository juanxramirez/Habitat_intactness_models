...
num_vars <- sapply(ext.risk.mammals.na.omit, is.numeric)
ext.risk.mammals.data.scaled <- ext.risk.mammals.na.omit
ext.risk.mammals.data.scaled[, num_vars] <- scale(ext.risk.mammals.na.omit[, num_vars])

summary(ext.risk.mammals.data.scaled)
...
data.cm<-data.frame(or=ext.risk.mammals.data.scaled$order,
                    fam=ext.risk.mammals.data.scaled$family,
                    gen=ext.risk.mammals.data.scaled$genus,
                    cm=ext.risk.mammals.data.scaled$continuum_p50,
                    rs=ext.risk.mammals.data.scaled$range_size,
                    gl=ext.risk.mammals.data.scaled$gestation_length,
                    wa=ext.risk.mammals.data.scaled$weaning_age,
                    rl.cat=ext.risk.mammals.data.scaled$rl_cat)
...
for (i in 1:k) {
  ...
  model.pm <- glmer(rl.cat~pm+rs+gl+wa+(1|or), family=binomial, data=learn.pm, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
...
for (i in 1:k) {
  ...
  model.cm <- glmer(rl.cat~cm+rs+gl+wa+(1|or), family=binomial, data=learn.cm, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
...
for (i in 1:k) {
  ...
  model.hm <- glmer(rl.cat~hm+rs+gl+wa+(1|or), family=binomial, data=learn.hm, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
...
write.table(auc.pmcmhm, file="auc_10-fold_cv_pmcmhm_specialists.csv", sep="\t", row.names=F)
