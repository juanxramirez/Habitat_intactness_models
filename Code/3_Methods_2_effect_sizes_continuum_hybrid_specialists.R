...
specialists[,7] <- ifelse(specialists[,7] == "LC", 0,
                           ifelse(specialists[,7] == "NT", 0,
                                  ifelse(specialists[,7] == "VU", 1,
                                         ifelse(specialists[,7] == "EN", 1,
                                                ifelse(specialists[,7] == "CR", 1, NA)))))

# Convert categorical variables to factor
specialists$order<-as.factor(specialists$order)
specialists$family<-as.factor(specialists$family)
specialists$genus<-as.factor(specialists$genus)
...
specialists.na.omit<-na.omit(specialists[,vars])

# Create data frame for extinction risk (ER) models
data.specialists<-data.frame(or=specialists.na.omit$order,
                             fam=specialists.na.omit$family,
                             gen=specialists.na.omit$genus,
                             cp5=specialists.na.omit$continuum_p5,
                             cp10=specialists.na.omit$continuum_p10,
                             cp50=specialists.na.omit$continuum_p50,
                             cp90=specialists.na.omit$continuum_p90,
                             cp95=specialists.na.omit$continuum_p95,
                             hp5=specialists.na.omit$hybrid_p5,
                             hp10=specialists.na.omit$hybrid_p10,
                             hp50=specialists.na.omit$hybrid_p50,
                             hp90=specialists.na.omit$hybrid_p90,
                             hp95=specialists.na.omit$hybrid_p95,
                             rs=specialists.na.omit$range_size,
                             gl=specialists.na.omit$gestation_length,
                             wa=specialists.na.omit$weaning_age,
                             rl.cat=specialists.na.omit$rl_cat)
...
#----
# ER ~ continuum (c) + ...
cp5.glmer<-glmer(rl.cat~cp5+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
cp10.glmer<-glmer(rl.cat~cp10+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
cp50.glmer<-glmer(rl.cat~cp50+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
cp90.glmer<-glmer(rl.cat~cp90+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
cp95.glmer<-glmer(rl.cat~cp95+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
...
c.effectsizes.table$habitat_breadth <- "Specialists"

#----
# ER ~ hybrid (h) + ...
hp5.glmer<-glmer(rl.cat~hp5+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
hp10.glmer<-glmer(rl.cat~hp10+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
hp50.glmer<-glmer(rl.cat~hp50+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
hp90.glmer<-glmer(rl.cat~hp90+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
hp95.glmer<-glmer(rl.cat~hp95+rs+gl+wa+(1|or), family=binomial, data=data.specialists.scaled, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
...
h.effectsizes.table$habitat_breadth <- "Specialists"

# Combine effect sizes for continuum and hybrid together
hc.effectsize.table<-rbind(c.effectsizes.table, h.effectsizes.table)

# Save effect sizes
write.table(hc.effectsize.table, file="effectsize_cmhm_specialists.csv", sep="\t", row.names=F)
